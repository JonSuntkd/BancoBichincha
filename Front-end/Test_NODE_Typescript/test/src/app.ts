...
export class Producto {
  ...
  constructor(nuevoProducto: string, precioProducto: number) {
    this.id = newUID();
  ...
  listarProducto(): void {
    console.log(
      `\t || Nombre: ${this.nombre} \t|| Precio : ${this.precio} \t|| Cantidad : ${this.cantidad} ||`
    );
  }
}
...
export class Factura {
  ...
  agregarProducto(producto: Producto, cantidadProducto: number): void {
    if (this.productoExistente(producto.nombre, cantidadProducto)) {
      // console.log(
      //   "El producto ya existe en la factura y se agrego la cantidad adicional"
      // );
    } else {
  ...
  productoExistente(nombreProduc: string, cantidad: number): boolean {
...
export function productoExistenteFactura(
  nombreProduc: string,
  nombre: string
): boolean {
  if (nombre === nombreProduc) {
    return true;
  }
  return false;
}
export function pExistenteFactura(
  productos: string[],
  nombre: string
): boolean {
